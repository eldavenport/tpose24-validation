"""
Extract UVEL, VVEL, DRHODR at 0N/1N/1S, 140W for all 138 model levels (to ~5450 m).
Saves to cache_edj_full.nc alongside the existing shallow cache_edj.nc.

diag_state field order: THETA=0, SALT=1, UVEL=2, VVEL=3, WVEL=4, PHIHYD=5, DRHODR=6

Usage:
    conda run -n tpose python build_edj_fulldepth_cache.py <SUBFOLDER>
...
CACHE_FILE = os.path.join(SAVE_DIR, 'cache_edj_full.nc')

os.makedirs(SAVE_DIR, exist_ok=True)

NZ, NY, NX = 138, 384, 512
nz_use     = 138   # all levels, deepest ≈ -5450 m
...
print(f'Files     : {n_t}')
print(f'nz_use    : {nz_use}  (all levels, to ~5450 m)')
...
# Mask below-seafloor cells: MITgcm writes 0.0 for land/below-seafloor.
# Replace with NaN where velocity is identically 0 at all times (below seafloor).
for arr in [U_0N, U_1N, U_1S, V_0N, V_1N, V_1S]:
    always_zero = np.all(arr == 0.0, axis=0)  # (depth,)
    # Only mask levels below the deepest non-zero level
    if np.any(~always_zero):
        deepest_active = np.where(~always_zero)[0][-1]
        arr[:, deepest_active + 1:] = np.nan
